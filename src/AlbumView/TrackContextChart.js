...
function TrackContextChart({tracks}) {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      const acousticnessValues = tracks.map((track) => {
        if (!('audioFeatures' in track))
          return null;
        return track.audioFeatures.acousticness
      });

      const livenessValues = tracks.map((track) => {
        if (!('audioFeatures' in track))
          return null;
        return track.audioFeatures.liveness
      });

      const chartData = [
        { x: 1, y: acousticnessValues },
        { x: 2, y: livenessValues }
      ];
  ...
  return (
    ...
    <VictoryChart
      ...
      <VictoryBoxPlot
        boxWidth={25}
        data={chartData}
        categories={{ x: ['acousticness', 'liveness'] }}
      />
...
export default TrackContextChart;
